...
// mass_index_reversal_daily at +0.51% mean). Regenerated by the offline
// audit pipeline; drop the timestamped JSON in at data/.
...
const agentLastSpec = ref(null);  // last PASS's emitted spec — for print (`spec`) + download
...
// Download the last emitted spec as <name>.json — the same artifact the Single
// token view's "Download spec" button and the CLI `--emit-spec` produce.
function downloadAgentSpec() {
  const spec = agentLastSpec.value;
  if (!spec) return;
...
function agentRenderVerdict(r) {
  ...
  if (r.spec?.name) {
    agentLastSpec.value = r.spec;
    pushAgent(`  ↳ backtestable spec emitted: ${r.spec.name}  —  type 'spec' to print it, or use ⬇ spec`, "info");
  }
...
async function onAgentSubmit(command) {
  ...
  // `spec` — print the last emitted strategy-spec JSON so it's copyable here.
  if (/^\s*(spec|show spec|print spec)\s*$/i.test(command)) {
    if (agentLastSpec.value) {
      pushAgent(`// ${agentLastSpec.value.name} — @stratchai/strategy-spec (validates + runs in @stratchai/backtest)`, "success");
      pushAgent(JSON.stringify(agentLastSpec.value, null, 2), "info");
    } else {
      pushAgent("No spec yet — evaluate a token that PASSes first (e.g. 'evaluate ETH as of 2025-09-28').", "warn");
    }
    return;
  }
  ...
  if (parsed.kind === "help") {
    ...
    pushAgent("  spec   — print the last PASS's backtestable strategy-spec JSON", "info");
    return;
  }
...
const honestDisclosure = computed(() => {
  ...
  if (r.verdict === "PASS") {
    return {
      ...
      bullets: [
        "Rare by design: in a trailing-90-day re-check of the CMC top-30 (scanned 2026-06-20), only TON qualified — 5 entries in late May, most recent 2026-05-25; the other 29 never matched. The tool won't fabricate a PASS just because you asked. The demo presets use historical dates where the oversold-in-uptrend setup genuinely occurred, to show what a real match looks like.",
    ...
    <div v-if="mode === 'agent'" class="max-w-7xl mx-auto px-6 py-6">
      <div class="mb-3 flex items-center justify-between flex-wrap gap-3">
        ...
        <div class="flex gap-2 flex-wrap items-center">
          ...
          <button
            @click="downloadAgentSpec"
            :disabled="!agentLastSpec"
            class="px-2.5 py-1 text-xs rounded border border-emerald-300 bg-emerald-50 text-emerald-800 hover:bg-emerald-100 font-mono disabled:opacity-40"
            title="Download the last PASS's backtestable @stratchai/strategy-spec JSON"
          >
            ⬇ spec
          </button>
        </div>
    ...
    <div v-if="mode === 'single' && result" class="max-w-7xl mx-auto px-6 pb-12">
      <div :class="['rounded-xl border bg-white shadow-sm overflow-hidden', cardBorder]">
        ...
        <!-- Audit transparency: 46 shelved daily-crypto archetypes evaluated
             ...
             from the bundled audit_universe_screening.json (regenerated by
             the offline audit pipeline). -->
        <div v-if="tokenAuditRows.length" class="px-6 py-5 border-b border-gray-100">
          ...
          <div v-if="auditExpanded" class="mt-3">
            <!-- Snapshot framing — load-bearing for honesty. The numbers in
                 this table are NOT computed live; they're the per-archetype,
                 per-token results from a single offline walk-forward audit run
                 on {{ auditMeta.scanned_at }}. Pooled stats are universe-wide;
                 per-token stats are this token's slice of that run. Both come
                 from the same timestamped JSON. -->
            <div class="bg-amber-50 border border-amber-200 rounded-md p-3 mb-3 text-xs text-amber-900 leading-relaxed">
              ...
              This is a fixed snapshot; refreshing it re-runs the offline audit pipeline.
            </div>

            <!-- Glossary — terminology audit. "shelved", "culled", "RESCUE"
                 all carry project-specific meaning that hackathon judges have
